        ...
        match winit_event {
            Event::WindowEvent {
                ...
                ScaleFactorChanged {
                    ...
                    self.raw_input.viewport().native_pixels_per_point = Some(*scale_factor as f32);
                    let inner_rect = self.raw_input.viewport().inner_rect.unwrap();
                    self.raw_input.screen_rect = Some(egui::Rect::from_min_size(
                        Default::default(),
                        vec2(inner_rect.max.x - inner_rect.min.x as f32, inner_rect.max.y - inner_rect.min.y as f32)
                            / self.scale_factor as f32,
                    ));
                    inner_size_writer.request_inner_size(PhysicalSize::new(
                        self.raw_input.screen_rect.unwrap().width() as u32,
                        self.raw_input.screen_rect.unwrap().height() as u32,
                    ));
                }
